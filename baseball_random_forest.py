...
customs_weights = {0: 1, 1: 5, 2: 15}

random_forest = RandomForestClassifier(
    n_estimators=100,
    min_samples_leaf=1,  # Número mínimo de amostras em um nó folha
    max_features='sqrt',  # Número de atributos a considerar em cada divisão
    class_weight= customs_weights,
    random_state=42,
)
...
feature_importances = pd.Series(
    random_forest.feature_importances_, index=X_train.columns
).sort_values(ascending=False)

print('--- Principais Atributos para o Modelo ---')
print(feature_importances)

# Plotar gráfico dos 10 atributos mais importantes
